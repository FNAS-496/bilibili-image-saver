...
1. **安装脚本管理器**（如 Tampermonkey），并导入 `bilibili-save.user.js`（只需做一次）
2. **双击 `一键保存.bat`**（中文界面）或 **`start_server.bat`**（English UI）启动本地服务（会打开浏览器）
3. **打开 B 站页面**：收藏夹 / 动态 / 作品页 → 自动开始下载，图片出现在保存目录
